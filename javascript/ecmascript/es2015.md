...
WeakMaps keys must be objects, the values can be anything. 
...
WeakSets are similar to week maps, they only store object. The objects are garbage collected. You cannot enumerate a WeakSet. A week set had three methods, `add`, `set`, and `delete`. 
